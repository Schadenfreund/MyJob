...
class NotesStatisticsMarkdownService {
  ...
  static String _generateMarkdown(List<NoteItem> notes,
    ...
    buffer.writeln();
    buffer.writeln(isGerman
        ? '**Legende:** 🔴 Dringend | 🟠 Hoch | 🟡 Mittel | 🔵 Niedrig | ✅ Erledigt | 📌 Aktiv | 📦 Archiviert'
        : '**Legend:** 🔴 Urgent | 🟠 High | 🟡 Medium | 🔵 Low | ✅ Completed | 📌 Active | 📦 Archived');
  ...
  static void _writeSummary(
    ...
    if (isGerman) {
      buffer.writeln(
          'Dieser Bericht enthält eine Übersicht über ${stats['total']} Notizen.');
      buffer.writeln(
      ...
      if ((stats['overdue'] as int) > 0) {
        buffer.writeln();
        buffer.writeln(
            '**Überfällige Einträge:** ${stats['overdue']}');
      }
    } else {
      buffer.writeln(
  ...
  static void _writeAllNotes(
    ...
    buffer.writeln(isGerman
        ? '| Erstellt | Typ | Titel | Priorität | Fällig | Status |'
  ...
  static void _writeStatistics(
    ...
    final priorityData = [
      ('🔴 Urgent', '🔴 Dringend', stats['urgent']),
      ('🟠 High', '🟠 Hoch', stats['high']),
      ('🟡 Medium', '🟡 Mittel', stats['medium']),
      ('🔵 Low', '🔵 Niedrig', stats['low']),
    ];
